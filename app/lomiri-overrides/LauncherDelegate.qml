...
Item {
    ...
    // HomeSpike: the home is portrait-locked, so the launcher renders portrait
    // while the phone is physically landscape — its icons would appear sideways.
    // Read the device angle from the same sensor the home screen/dock use and
    ...
    ShaderEffect {
        ...
        transform: [
            ...
            // HomeSpike: spin the composited icon upright for the physical device
            // orientation (Z-axis, around the icon centre) — applied last so it
            // turns the whole folded/scaled icon. The fold above is X-axis, so
            // the two compose without fighting.
            Rotation {
                id: deviceOrientationRotation
                axis { x: 0; y: 0; z: 1 }
                origin { x: iconItem.width / 2; y: iconItem.height / 2; z: 0 }
                angle: root.deviceAngle
                Behavior on angle { RotationAnimation { duration: 250; direction: RotationAnimation.Shortest } }
            }
        ]
