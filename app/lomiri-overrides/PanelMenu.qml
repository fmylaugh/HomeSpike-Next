...
Showable {
    ...
    // HomeSpike: optional inset (px) on the icon row's right edge so a rounded
    // screen corner doesn't crop the rightmost indicator (the clock). Only the
    // bar shifts — the pull-down menu and drag handle stay full-width. Default
    // 0 ⇒ stock, so other PanelMenu users (the application menus) are untouched.
    property real barRightInset: 0
    ...
    Rectangle {
        anchors.fill: bar
        // HomeSpike: the bar may be inset (barRightInset) so its icons clear a
        // rounded corner, but this opaque backing must still span the full top
        // strip — otherwise the uncovered corner shows the blurred background
        // when the menu is open. Cancel the inset on the right edge only.
        anchors.rightMargin: -root.barRightInset
        color: panelColor
        visible: !root.fullyClosed
    }
    ...
    PanelBar {
        ...
        anchors {
            left: parent.left
            right: parent.right
            rightMargin: root.barRightInset   // HomeSpike: rounded-corner inset
        }
        expanded: false
